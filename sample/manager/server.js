...
const httpServerRequest = (req, res) => {
...
function getContentType(fpath) {
	var ext = fpath
		...
		.pop()
		.toLowerCase();
	console.log("----getContenttype-----", ext);
...
// "name":"IP Camera",
// 	"xaddrs": [
// 		"http://192.168.4.40/onvif/device_service"
// 	]
// }
// http: //192.168.4.40:53333/videostream.cgi?user=admin&pwd=123456789
...
function startDiscovery(conn) {
	...
	let odevice = new onvif.OnvifDevice({
		xaddr: "http://192.168.4.40:10080/onvif/device_service",
		user: "admin",
		pass: "123456789"
	});
	...
	console.log("----devices---->", devices, res);
	conn.send(JSON.stringify(res));
}

function connect(conn, params) {
	params.user = "admin";
	params.pass = "123456789";
	// //console.log('devices--->', devices);
	...
	if (!device) {
		var res = {
			id: "connect",
			error: "The specified device is not found A: " + params.address
		};
		conn.send(JSON.stringify(res));
		return;
	}
	...
	device.init((error, result) => {
		...
		if (error) {
			...
			console.log('--device init error--->', err);
			res["error"] = err;
		} else {
			res["result"] = result;
		}
...
function fetchSnapshot(conn, params) {
	...
	if (!device) {
		var res = {
			id: "fetchSnapshot",
			error: "The specified device is not found B: " + params.address
		};
		conn.send(JSON.stringify(res));
		return;
	}
	device.fetchSnapshot((error, result) => {
		...
		if (error) {
			let err = error.toString();
			console.log('----fetchSnashot--err---->', err);
			res["error"] = err;
		} else {
...
function ptzMove(conn, params) {
	...
	if (!device) {
		var res = {
			id: "ptzMove",
			error: "The specified device is not found C: " + params.address
		};
		conn.send(JSON.stringify(res));
		return;
	}
...
function ptzStop(conn, params) {
	...
	if (!device) {
		var res = {
			id: "ptzStop",
			error: "The specified device is not found D: " + params.address
		};
		conn.send(JSON.stringify(res));
		return;
	}
...
function ptzHome(conn, params) {
	...
	if (!device) {
		var res = {
			id: "ptzMove",
			error: "The specified device is not found E: " + params.address
		};
		conn.send(JSON.stringify(res));
		return;
	}
